...
const CertificateCard = ({ cert, onView }) => (
  <div className="border border-gray-100 rounded-2xl overflow-hidden hover:shadow-md transition group">
    {/* Gold header bar */}
    <div className="bg-gradient-to-r from-yellow-400 to-amber-500 px-4 py-3 flex items-center justify-between">
      <div className="flex items-center gap-2">
        <span className="text-2xl">🏆</span>
        <span className="text-white font-bold text-sm">Certificate of Completion</span>
      </div>
      <span className="text-xs bg-white/30 text-white px-2 py-0.5 rounded-full font-semibold">
        Issued
      </span>
    </div>
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-400 to-indigo-500 flex items-center justify-center text-white font-bold text-sm flex-shrink-0 overflow-hidden shadow">
          {cert.course?.thumbnail ? (
            <img src={getImageSrc(cert.course.thumbnail)} alt={cert.course.title}
              className="w-full h-full object-cover"
              onError={(e) => { e.target.style.display = 'none'; }} />
          ) : cert.course?.title?.charAt(0)}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-gray-900 truncate">{cert.course?.title}</p>
          <p className="text-xs text-gray-400">by {cert.teacher?.name || 'Instructor'}</p>
        </div>
        {cert.grade != null && (
          <div className="text-right flex-shrink-0">
            <p className="text-lg font-bold text-amber-600">{cert.grade}%</p>
      ...
      <div className="bg-amber-50 border border-amber-100 rounded-xl px-3 py-2 mb-3">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-amber-700 font-semibold">Certificate No.</p>
            <p className="text-xs font-mono text-amber-900 font-bold">{cert.certificateNumber}</p>
          </div>
          <div className="text-right">
            <p className="text-xs text-amber-700 font-semibold">Completed</p>
      ...
      <div className="flex gap-2">
        <button
          onClick={() => onView(cert)}
          className="flex-1 py-2 bg-gradient-to-r from-amber-500 to-yellow-500 text-white rounded-xl text-xs font-bold hover:opacity-90 transition"
        >
          👁 View Certificate
        </button>
        <button
          onClick={() => navigator.clipboard.writeText(cert.verificationCode).then(() => alert('Verification code copied!'))}
          title="Copy verification code"
          className="w-9 h-9 flex items-center justify-center rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-500 transition text-sm"
        >
          📋
        </button>
      </div>
