...
const handleLogoRemove = async () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) notify('success', `${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} page updated successfully!`, 3000);
      else notify('error', 'Failed to update content');
            ...
            <div className={sectionCard}>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {aboutContent.stats.map((stat, i) => (
                  <div key={i} className="border border-slate-200 rounded-xl p-4 grid grid-cols-2 gap-3">
            ...
            <div className={sectionCard}>
              ...
              <div className="space-y-4">
                ...
                {aboutContent.storyParagraphs.map((p, i) => (
                  <div key={i}><label className="block text-sm font-medium text-slate-700 mb-2">Paragraph {i + 1}</label><textarea value={p} onChange={e => updateStoryParagraph(i, e.target.value)} rows={3} className={inp3} /></div>
                ))}
              </div>
            </div>
            <div className={sectionCard}>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {aboutContent.values.map((v, i) => (
                  <div key={i} className="border border-slate-200 rounded-xl p-4 space-y-3">
                    <div><label className="block text-xs font-medium text-slate-500 mb-1.5">Icon</label><input type="text" value={v.icon} onChange={e => updateValue(i, 'icon', e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-slate-900/10 transition-all" /></div>
            ...
            <div className={sectionCard}>
              ...
              <div className="space-y-4">
                {contactContent.faqs.map((faq, i) => (
                  <div key={i} className="border border-slate-200 rounded-xl p-4 space-y-3">
                    <div><label className="block text-sm font-medium text-slate-700 mb-2">Question {i + 1}</label><input type="text" value={faq.question} onChange={e => updateFAQ(i, 'question', e.target.value)} className={inp} /></div>
                    <div><label className="block text-sm font-medium text-slate-700 mb-2">Answer</label><textarea value={faq.answer} onChange={e => updateFAQ(i, 'answer', e.target.value)} rows={2} className={inp3} /></div>
                  </div>
                ))}
              </div>
            </div>
                    ...
                    <div key={msg._id} className="px-6 py-5 hover:bg-slate-50/50 transition-colors group">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <button onClick={() => deleteMessage(msg._id)}
                          className="opacity-0 group-hover:opacity-100 inline-flex items-center gap-1 text-rose-600 hover:text-white hover:bg-rose-600 bg-rose-50 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all flex-shrink-0">
